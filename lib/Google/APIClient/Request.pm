...
#    has '_json_handle' => ( isa => 'JSON',           is => 'ro', builder => '__build_json_handle'  );
    ...
    # builders
    action __build_lwp_handle() { return Google::APIClient::UserAgent->new(); }

#    action __build_json_handle() { return JSON->new->allow_nonref; }
...
#      def http_method=(new_http_method)
    action set_http_method($new_http_method) {
...
#      def api_method=(new_api_method)
    action set_api_method($new_api_method) {
...
#          upload = result.resumable_upload
            my $upload = $result->resumable_upload;
#          unless upload.complete?
            unless ( $upload->is_complete ) {
#            logger.debug { "#{self.class} Sending upload body" }
                print "#{self.class} Sending upload body\n";
...
#    def to_env(connection)
    action to_env($connection) {
#        method, uri, headers, body = self.to_http_request
        my ( $method, $uri, $headers, $body ) = $self->to_http_request();
        my $http_request = $connection->build_request($method,$uri,$headers,$body);
#        http_request = connection.build_request(method) do |req|
#          req.url(uri.to_s)
#          req.headers.update(headers)
#          req.body = body
#        end
        $body = $self->to_http_request();
#
...
#        request_env = http_request.to_env(connection)
        my $request_env = $http_request->to_env($connection);
...
#    def process_http_response(response)
    action process_http_response($response) {
...
#    def initialize_media_upload(options)
    action initialize_media_upload($options) {
...
#    def build_multipart(parts, mime_type = 'multipart/related', boundary = MULTIPART_BOUNDARY)
    action build_multipart($parts, $mime_type, $boundary) {
#        env = Faraday::Env.new
